...
class scatter_term(ManagerTermBase):
    ...
    def __init__(self, cfg: ManagerTermBaseCfg, env: ManagerBasedRLEnv):
        """Pre-allocate the shared output buffer for all child terms.

        The output dimension comes from ``cfg.params["output_dim"]`` or, if
        absent, from the first child's ``output_dim`` attribute; a buffer of
        shape ``(num_envs, output_dim)`` (or ``(num_envs,)`` for
        ``output_dim=0``) is allocated once.  Raises :class:`ValueError` when
        neither source provides a dimension.
        """
    ...
    def __call__(
        ...
        """Evaluate each child term for its group's envs and scatter into the buffer.

        The buffer is zeroed, then each ``@scatterable`` child writes only its
        clone group's rows via ``_out=buf``; envs outside every group stay
        zero.  The ``terms``/``output_dim`` arguments only mirror ``cfg.params``
        for the manager's signature check.  Returns the shared buffer of shape
        ``(num_envs, output_dim)`` (or ``(num_envs,)`` for ``output_dim=0``).
        """
        self._buf.zero_()
