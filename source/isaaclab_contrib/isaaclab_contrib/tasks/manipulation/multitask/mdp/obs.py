...
class MultiTaskObsTerm(ObsTerm):
    ...
    def __post_init__(self):
        """Wrap ``func`` with a ``(num_envs, dim)`` zero fallback when ``dim`` is set."""
...
class multi_task_onehot(ManagerTermBase):
    ...
    def __init__(self, cfg: ManagerTermBaseCfg, env: ManagerBasedRLEnv):
        """Precompute the per-env one-hot of the clone-group index.

        Each env's row encodes which registration (clone group) it belongs to;
        the width equals the number of registrations.  Group membership is
        static, so the encoding is built once and cached.
        """
    ...
    def __call__(self, env: ManagerBasedRLEnv) -> torch.Tensor:
        """Return the cached one-hot encoding. Shape is ``(num_envs, num_selectors)``."""
        return self._onehot
