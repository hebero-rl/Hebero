...
class PoseCommand(CommandTerm):
    ...
    def __init__(self, cfg: PoseCommandCfg, env: ManagerBasedRLEnv):
        """Allocate full-env command buffers and resolve the selector-scoped asset.

        The ``(num_envs, 7)`` command buffers always exist (identity quaternion
        rows) so obs terms can read them safely.  When the selector resolves to
        an empty env set (disabled group, ``weight=0``), the term enters a
        no-op mode and never touches the missing asset.
        """
    ...
    def __str__(self) -> str:
        """Return a summary string with command shape, resampling range, and selector."""
        msg = "PoseCommand:\n"
    ...
    def _update_metrics(self):
        """Update world-frame command/EE poses and pose-error metrics.

        Writes only the selected rows: ``metrics["position_error"]`` [m] and
        ``metrics["orientation_error"]`` [rad] are ``(num_envs,)`` tensors with
        non-selected envs left at zero.  No-op when the group is disabled.
        """
        if self._disabled:
            return
    ...
    def _resample_command(self, env_ids: torch.Tensor):
        """Sample new uniform pose commands for the given envs.

        Filters ``env_ids`` down to this term's selector group; positions [m]
        and Euler-derived quaternions are drawn from ``cfg.ranges`` and written
        into the matched rows of the root-frame command buffer only — other
        envs' rows are untouched.  No-op when the group is disabled.
        """
        if self._disabled:
            return
    ...
    def _update_command(self):
        """Do nothing — the pose command is constant between resamples."""
        pass

    def _set_debug_vis_impl(self, debug_vis: bool):
        """Create (on first use) and toggle the goal/EE frame marker visualizers."""
        if debug_vis:
    ...
    def _debug_vis_callback(self, event):
        """Draw goal and current EE frame markers at their world-frame poses.

        No-op when the group is disabled or the robot is not yet initialized.
        """
        if self._disabled:
            return
