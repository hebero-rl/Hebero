...
def joint_vel_l2(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> ScatterResult:
    """Joint velocity L2 penalty: sum of squared joint velocities [(rad/s)^2 or (m/s)^2].

    Evaluated only for the asset's clone-group envs and scattered into a
    ``(num_envs,)`` tensor; other envs stay zero.
    """
...
def position_command_error(
    ...
    """Position command tracking L2 error [m]: distance between EE body and commanded position.
...
def object_ee_distance(
    ...
    *,
    ...
    """Object-to-EE distance with tanh kernel: ``1 - tanh(dist / std)`` in [0, 1].

    ``std`` [m] sets the distance scale over which the reward decays.
    Evaluated only for the clone-group envs and scattered into a
    ``(num_envs,)`` tensor; other envs stay zero.
    """
...
def object_is_lifted(
    ...
    *,
    ...
    """Object lifted above threshold: 1.0 when object height exceeds ``minimal_height`` [m], else 0.0.
...
def cabinet_approach_ee_handle(
    ...
    *,
    ...
    """Reward for reaching the cabinet handle: inverse-square kernel of EE-to-handle distance.

    ``(1 / (1 + d^2))^2`` in (0, 1], doubled once the distance drops below
    ``threshold`` [m].  Evaluated only for the clone-group envs and scattered
    into a ``(num_envs,)`` tensor; other envs stay zero.
    """
    ee_pos = wp.to_torch(env.scene[ee_frame_cfg.name].data.target_pos_w)[ee_frame_cfg.view_ids, 0, :]
...
def cabinet_open_drawer_bonus(
    ...
    """Drawer opening bonus: drawer joint position [m], doubled while the fingers straddle the handle.
...
def cabinet_multi_stage_open_drawer(
    ...
    """Multi-stage drawer opening bonus with milestones at 0.01 m, 0.2 m, and 0.3 m drawer opening.

    0.5 for opening past 0.01 m, plus 1.0 each for passing 0.2 m and 0.3 m
    while the fingers straddle the handle (max 2.5).  Evaluated only for the
    clone-group envs and scattered into a ``(num_envs,)`` tensor; other envs
    stay zero.
    """
    cabinet = env.scene[cabinet_asset_cfg.name]
    drawer_pos = wp.to_torch(cabinet.data.joint_pos)[cabinet_asset_cfg.view_ids, cabinet_asset_cfg.joint_ids].squeeze(
