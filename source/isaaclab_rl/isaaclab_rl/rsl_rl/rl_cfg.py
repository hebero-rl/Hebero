...
@configclass
class RslRlTokenPoolModelCfg(RslRlMLPModelCfg):
    """Configuration for the token-pooling (ViT-perception) model.

    The pooled groups carry no extra hyper-parameters -- a single-query attention pool is
    fully determined by the token width the environment publishes -- so this differs from
    :class:`RslRlMLPModelCfg` only in which class it selects.
    """

    class_name: str = "isaaclab_rl.rsl_rl.models:TokenPoolModel"
    """The model class name. Defaults to Isaac Lab's :class:`~isaaclab_rl.rsl_rl.models.TokenPoolModel`."""
...
class RslRlOnPolicyRunnerCfg(RslRlBaseRunnerCfg):
    ...
    history_checkpoint_step: int = 2000
    """Iteration granularity for retained checkpoints; ``<= 0`` disables pruning.

    Periodic ``save_interval`` saves still happen (crash recovery); after each one
    the runner keeps milestones at this granularity, the most recent intermediate,
    and ``model_final.pt``.  See :mod:`isaaclab_rl.rsl_rl.checkpoint_pruning`.

    A 2000-iteration interval retains 15 milestones over a 30k-iteration run,
    providing checkpoints for evaluation throughout training."""
