...
class RslRlVecEnvWrapper(VecEnv):
    ...
    def step(self, actions: torch.Tensor) -> tuple[TensorDict, torch.Tensor, torch.Tensor, dict]:
        ...
        # multi-task envs: merge per-task SR logs and the RLfD algorithm extras
        # (task ids / success / teacher actions) from the task-SR tracker that
        # the env's `task_success_rate` curriculum term maintains (duck-typed —
        # single-task envs have no tracker and skip this entirely).
        tracker = getattr(self.unwrapped, "task_sr_tracker", None)
        if tracker is not None:
            extras.update(tracker.step_extras(self.unwrapped))
            log = extras.setdefault("log", {})
            log.update(tracker.step_log())
