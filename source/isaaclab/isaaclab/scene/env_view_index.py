...
class EnvViewIndex:
    ...
    def __init__(self, cfg: SelectorCfg, *, num_envs: int, device: str = "cpu"):
        ...
        # asset name -> dense (num_envs,) table of rows into that asset's data buffer;
        # see _asset_row_lut. Invalidated with _view_cache whenever a layout changes.
        self._row_lut_cache: dict[str, torch.Tensor] = {}
    ...
    def _invalidate_caches(self) -> None:
        """Drop everything derived from the layouts / asset->group map.

        Both caches answer questions that are constant only *after* registration is
        finished. Registration all happens at scene construction today, so this never
        fires in a run -- it is here so that a layout change cannot silently be served
        a stale view or row table.
        """
        self._view_cache.clear()
        self._row_lut_cache.clear()
    ...
    def filter_reset_ids(self, asset_name: str, candidate_env_ids: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            ...
            The two are aligned elementwise and follow ``candidate_env_ids`` order.
            For an asset spanning several clone groups this used to come back grouped
            by group instead, which callers pairing the two tensors could not observe,
            but callers relying on alignment with an ascending env-id list (e.g. the
            LIBERO per-task gather in ``mdp/combined.py``) can: candidate order is the
            one they already assume.
        """
        ...
        rows = self._asset_row_lut(asset_name)[candidate_env_ids]
        owned = rows >= 0
        return candidate_env_ids[owned], rows[owned]

    def _asset_row_lut(self, asset_name: str) -> torch.Tensor:
        """Dense ``env id -> row in this asset's data buffer``, ``-1`` where unowned.

        :meth:`filter_reset_ids` used to answer this per call by walking the asset's
        clone groups and running a ``searchsorted`` plus two masked gathers per group,
        then a second pass to turn matched env ids into rows. Both the group layouts
        and the asset->group map are fixed once the scene exists, so the mapping is a
        run constant; materialising it once turns the filter into two kernels with no
        Python loop over groups. That matters because the callers are the reset path --
        :meth:`~isaaclab.scene.InteractiveScene.reset` runs one filter per scene entity
        per reset, and the demo-reset event one per asset -- so the loop cost scaled
        with (entities x groups-per-entity) on every episode boundary.

        Costs one int64 per env per asset (~1 MB for 40 assets at 3200 envs).
        """
        lut = self._row_lut_cache.get(asset_name)
        if lut is None:
            asset_env_ids = self._get_asset_env_ids(asset_name)
            lut = torch.full((self._num_envs,), -1, dtype=torch.long, device=self._device)
            lut[asset_env_ids] = torch.arange(asset_env_ids.numel(), dtype=torch.long, device=self._device)
            self._row_lut_cache[asset_name] = lut
        return lut
